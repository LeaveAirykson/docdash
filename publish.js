...
function itemIsExcluded(item) {
  var docdash = (env && env.conf && env.conf.docdash) || {};
  var exAcc = docdash.excludedAccess?.includes(item.access) ?? false;
  var exSco = docdash.excludeScopes?.includes(item.scope) ?? false;
...
function buildMemberNav(items, itemHeading, itemsSeen, linktoFn) {
  ...
  if (items && items.length) {
    ...
    items.forEach(function (item) {
      ...
      // exclude items by item access and scope
      ...
      if (!hasOwnProp.call(item, 'longname')) {
        ...
        if (
          ...
          members.forEach(function (member) {
            ...
            if (!docdash.includeAllMembers && !member.scope == 'static') {
              return;
            }
